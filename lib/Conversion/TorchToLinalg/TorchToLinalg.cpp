...
public:
  ...
  matchAndRewrite(AtenNativeLayerNormBackwardOp op, OpAdaptor adaptor,
    ...
    // TODO: Handle the None cases for the optional parameters:
    // weight, bias.
    if (failed(checkNotNone(rewriter, op, gradOut)) ||
        failed(checkNotNone(rewriter, op, input)) ||
        failed(checkNotNone(rewriter, op, mean)) ||
        failed(checkNotNone(rewriter, op, rSTD)) ||
        failed(checkNotNone(rewriter, op, weight)) ||
        failed(checkNotNone(rewriter, op, bias)))
      return failure();
    ...
    // Step 1. Calculate dWeight = reduce_sum(gradOut * x_hat) across dimensions
    // until `axis`.
